			...
			switch ($data ['step']) {
				...
				case 1 :
					...
					if ($form->isValid ( $data )) {
						...
						$form = new Form_Edges_Add_Step2 ( $data );
					}
					break;
				...
				case 2 :
					...
					if ($form->isValid ( $data )) {
						...
						$form = new Form_Edges_Add_Step3 ( $data );
				...
				case '3' :
					...
					if ($form->isValid ( $data ))
					{
						try{
							...
							$row = $t->createRow ( array ('id_node_from' => $data ['id_node_from'], 'id_node_to' => $data ['id_node_to'], 'duration_min' => $data ['duration_min'], 'duration_max' => $data ['duration_max'] ) );
							$row->save ();
							
							
							// Ajouter un message pour confirmé l'ajout du record dans la base de données
							$this->addMsg ( 'Arete ajoutée' );
