...
public function MergeField($NameLst,$Value='assigned',$IsUserFct=false,$DefaultPrm=false) {
	...
	foreach ($NameLst as $Name) {
		...
		case 'afficher': $this->meth_Merge_AutoOn($this->Source,'afficher',true,true);$Cont=true;
...
public function Show($Render=false) {
	...
	if ($Ok!==false) {
		$this->meth_Merge_AutoOn($this->Source,'afficher',true,true);
		...
		for ($i=$SubStart;$i<$Loc->SubNbr;$i++) {
			...
			if (is_array($Value)) {
				...
				if (method_exists($Value,$x)) {
					if ($this->MethodsAllowed or !in_array(strtok($Loc->FullName,'.'),array('onload','afficher','var')) ) {
...
// Merge [onload] or [afficher] fields and blocks
